...
const TypeChecker injectableChecker = TypeChecker.fromRuntime(Managed);
...
class DependencyResolver {
  ...
  Future<DependencyConfig> _resolveActualType(
    ...
    if (injectableAnnotation != null) {
      ...
      if (injectable.instanceOf(TypeChecker.fromRuntime(View))) {
        isView = true;
      }
      if (injectable.instanceOf(TypeChecker.fromRuntime(Controller))) {
        isController = true;
      }
    ...
    _dep.tag = tag;
    _dep.isView = isView;
    _dep.isController = isController;
