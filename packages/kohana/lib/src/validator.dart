...
class TextFieldValidator {
  ...
  TextFieldValidator(this.validator, {this.errorText, this.throws = true, this.ignoreEmptyValues = true});
...
class TextFieldMinLengthValidator extends TextFieldValidator {
  TextFieldMinLengthValidator(int min, {String errorText = "Minimum characters"})
      : super(MinLengthValidator(min), errorText: errorText);
}

class TextFieldMaxLengthValidator extends TextFieldValidator {
  TextFieldMaxLengthValidator(int max, {String errorText = "Maximum characters"})
...
class TextFieldMultiValidator extends TextFieldValidator {
  ...
  TextFieldMultiValidator(List<TextFieldValidator> validators, {bool throws = true})
      : this.validators = validators.length != 0
            ? validators
            : [
...
class AppMultiValidator extends Validator<String> {
  final List<Validator> validators;

  AppMultiValidator(List<Validator> validators)
      : this.validators = validators.length != 0
            ? validators
            : [
                AlwaysValidValidator(),
...
class AppTextFieldMultiValidator extends TextFieldValidator {
  final List<TextFieldValidator> validators;
  String errorText = '';

  AppTextFieldMultiValidator(List<TextFieldValidator> validators, {bool throws = true})
      : this.validators = validators.length != 0
            ? validators
            : [
                TextFieldAlwaysValidValidator(),
...
class GenericFormFieldValidator extends Validator<String> {
  final FormFieldValidator<String> validator;

  GenericFormFieldValidator(this.validator);

  @override
  bool isValid(String value) => validator(value) == null;
}
